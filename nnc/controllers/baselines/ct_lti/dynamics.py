...
class ContinuousTimeInvariantDynamics(ControlledDynamics):
    def __init__(self,
        ...
        :param interaction__matrix: The interaction matrix `A`, that determines how states of `n`
        nodes interact, shape `n x n`.
        :param driver_matrix: The driver matrix B, which determines how `k` control signals are
        applied in the linear dynamics, shape `n x k`.
    ...
    def forward(self, t, x, u=None):
        ...
        dx = torch.matmul(self.interaction__matrix, x.unsqueeze(-1)).squeeze()

        if u is not None:
            # if control signals are provided, calculate matrix product `<B,u>`
            control_term = torch.matmul(self.driver_matrix, u.unsqueeze(-1)).squeeze()
            # add both parts
            dx += control_term
