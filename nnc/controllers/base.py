...
class BaseController(ABC, nn.Module):
    ...
    def forward(self, t: torch.Tensor, x: torch.Tensor) -> torch.Tensor:
        ...
        `b` dimensions for batch size, `m` dimensions for state variables and `n` dimensions for
        number of nodes.
        ...
        n`-dimensional state `x` to calculate control signals `u` of arbitrary dimensionality.
        """
        pass
...
class ControlledDynamics(torch.nn.Module):
    def __init__(self, state_var_list: list):
        ...
        `m x n`, for `n` nodes and `m` = 2 state variables:
    ...
    def forward(self, t, x, u=None) -> torch.Tensor:
        ...
        E.g. for a scalar control signal per node, shape: `[b, m, n]`.
