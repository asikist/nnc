...
class TrainingAlgorithm:
    ...
    def train(self, theta_0, total_time, epochs=9, lr=1, allow_logging=False, calculate_loss = calculate_loss):
        """
...
class FinalStepLossTrainingAlgorithm:
    def __init__(self, net, dynamics, calculate_loss):
        self.nnc_dyn = NNCDynamics(dynamics, net)
        self.neural_net = net
        self.optimizer = None
    ...
    def train(self, theta_0, total_time, epochs=9, lr=1):
        ...
        if len(theta_0.shape) == 1:
            x0 = x0.unsqueeze(1)
        t = torch.linspace(0, total_time, 2)
        self.optimizer = torch.optim.LBFGS(self.nnc_dyn.parameters(), lr=lr, max_iter=2, max_eval=None, history_size=10)
        # optimizer = torch.optim.LBFGS(nnc_dyn.parameters(), lr=0.00001)
        ...
        for i in range(epochs):
            ...
            try:
                def closure():
                    ...
                    loss = self.calculate_loss(x_total_time)
                    logging.info('Current loss is: ' + str(loss.item()))
                ...
                logging.info(e)
                self.optimizer = torch.optim.LBFGS(neural_net.parameters(), lr=lr, max_iter=2, max_eval=None, history_size=10)
        return best_model[0], best_loss[0]
